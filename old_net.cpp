...
Net::Net(Configuration config) {
    ...
    // Initialize Weights
    i2h_weights = matrix(config.input_size, vector<float>(config.hidden_layer_size, 0.0));
    h2o_weights = matrix(config.hidden_layer_size, vector<float>(config.output_size, 0.0));
    i2h_b = vector<float>(config.hidden_layer_size, 0.0);
    h2o_b = vector<float>(config.output_size, 0.0);

    // Initialize Random Number Generator
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_real_distribution<> dis(0.0, std::sqrt(2.0 / config.input_size));

    // Randomize Weights
    for (int i = 0; i < config.input_size; i++) {
        for (int j = 0; j < config.hidden_layer_size; j++) {
            i2h_weights[i][j] = dis(gen);
        }
    }

    for (int i = 0; i < config.hidden_layer_size; i++) {
        for (int j = 0; j < config.output_size; j++) {
            h2o_weights[i][j] = dis(gen);
        }
    }
}
...
// Forward Propagation
vector<float> Net::forward(vector<float> input) {
